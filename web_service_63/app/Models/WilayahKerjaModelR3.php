...
use App\Libraries\WilayahKerjaR3;
use App\Models\RutaModelR3;

class WilayahKerjaModelR3 extends Model
{
    ...
    public function getWilayahKerja($id)
    {
        ...
        if ($result != NULL) {
            ...
            $wilayah_kerja = new WilayahKerjaR3(
                $result['kode_bs'],
                $result['nama_bs'],
                $result['kode_desa'],
                $result['nama_desa'],
    ...
    public function getRutaCount(): array
    {
        $result = $this
            ->select('bloksensus.id, COUNT(*) as jumlah_ruta')
            ->join('rumahtangga', 'bloksensus.id = rumahtangga.kodeBs')
            ->groupBy('bloksensus.id')
            ->findAll();
    ...
    public function getJumlahTerkirim(string $kodeBs): int
    {
        ...
        SELECT t2.jumlah as jumlah_terkirim FROM 
            ...
            dkk.kabno as kode_kabupaten, dkk.nama as nama_kabupaten FROM 
                pkl62_wilayah_riset3.bloksensus dkb INNER JOIN 
                    pkl62_wilayah_riset3.desa dkd ON dkd.desano = dkb.kelurahandesa AND dkd.kecno = dkb.kecamatan 
                    AND dkd.kabno = dkb.kabupaten 
                INNER JOIN pkl62_wilayah_riset3.kecamatan dkc ON dkc.kecno = dkb.kecamatan AND dkc.kabno = dkb.kabupaten 
                INNER JOIN pkl62_wilayah_riset3.kabupaten dkk ON dkk.kabno = dkb.kabupaten 
            WHERE dkb.id = ?) t1
        LEFT OUTER JOIN
            ...
            FROM pkl62.notif n
            WHERE SUBSTRING(n.UploadName,1,2)='R3' AND n.status = 'Terkirim'
            GROUP BY bs) t2
