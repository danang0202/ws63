...
class WilayahKerjaModelSby extends Model
{
    ...
    public function getJumlahTerkirim(string $kodeBs): int
    {
        ...
        SELECT t2.jumlah as jumlah_terkirim FROM 
            ...
            dkk.kabno as kode_kabupaten, dkk.nama as nama_kabupaten FROM 
                pkl62_wilayah_surabaya_riset1.bloksensus dkb INNER JOIN 
                    pkl62_wilayah_surabaya_riset1.desa dkd ON dkd.desano = dkb.kelurahandesa AND dkd.kecno = dkb.kecamatan 
                    AND dkd.kabno = dkb.kabupaten 
                INNER JOIN pkl62_wilayah_surabaya_riset1.kecamatan dkc ON dkc.kecno = dkb.kecamatan AND dkc.kabno = dkb.kabupaten 
                INNER JOIN pkl62_wilayah_surabaya_riset1.kabupaten dkk ON dkk.kabno = dkb.kabupaten 
            WHERE dkb.id = ?) t1
        LEFT OUTER JOIN
            ...
            FROM pkl62_wilayah.notif n
            WHERE SUBSTRING(n.UploadName,1,2)='Sby' AND n.status = 'Terkirim'
            GROUP BY bs) t2
        ...
        if(isset($this->query($sql, [$kodeBs])->getResultArray()[0])){
